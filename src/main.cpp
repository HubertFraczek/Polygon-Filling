...
#pragma omp parallel for default(none) num_threads(NUMB_OF_TH) shared(map, accV2)
...
std::vector<int> scanLineRow_MPI(std::vector<int> row) {
	int xMax = row.size();
#pragma omp parallel for default(none) num_threads(NUMB_OF_TH) shared(xMax, row)
